...
const AttendanceSystem = props => {
  ...
  const attendanceResponse = useSelector(
    state => state.AttendanceReducer.attendanceResponse,
  );

  const isLoading = useSelector(state => state.CommonLoaderReducer.isLoading);

  useEffect(() => {
    if (typeof attendanceResponse === 'object' && attendanceResponse.length) {
      ...
      typeof attendanceResponse === 'object' &&
      Object.keys(attendanceResponse).length
    ) {
      dispatch(AttendanceActionPost.emptyAttendanceRequest());
      showMessage({
        style: { alignItems: 'flex-end' },
        type: 'danger',
        message: attendanceResponse.message.replace('None', ''),
      });
    }
  }, [attendanceResponse]);
  ...
  const handleAttendanceSystem = async () => {
    ...
    if (dateone && time) {
      ...
      timeErr = false;
      dateoneErr = false;
      dispatch(AttendanceActionPost.submitAttendance(data, accessToken));
        ...
        <View style={{ alignItems: 'center' }}>
          <View style={{ width: '90%' }}>
            ...
            <View style={styles.heading}>
              <Text style={{ fontFamily: '29LTAzer-Regular' }}>تاريخ</Text>
            </View>
            ...
            <View style={styles.heading}>
              <Text style={{ fontFamily: '29LTAzer-Regular' }}>زمن</Text>
            </View>
          ...
          <View style={{ width: '80%' }}>
            <CommonFormButton onPress={handleAttendanceSystem} {...props} />
          </View>
        </View>
      </ScrollView>
