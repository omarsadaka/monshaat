...
const SalaryMarchesGroup = (props) => {
  ...
  useEffect(() => {
    const unsubscribe = props.navigation.addListener('focus', () => {
      if (item) {
        ...
        let url =
          baseUrl +
          `/api/read/last_update?res_model=hr.payslip.run&res_id=${item.id}`;
        (async () => {
          ...
          >
            ...
            >
              ...
              <OrderViewItem
                title1="الإدارة"
                title2={
                  item.department_level1_id ? item.department_level1_id : '--'
                }
                icon={require('../../assets/images/order/id.png')}
                title2Style={{
                  backgroundColor: '#FFFFFF',
...
export default SalaryMarchesGroup;
