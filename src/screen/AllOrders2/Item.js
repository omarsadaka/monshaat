...
import ContractOrderDetail from '../MyRequest/DetialsView/ContractOrderDetail';
import NewLeaveRequest from '../MyRequest/DetialsView/NewLeaveRequest';
import MandateRequest from '../MyRequest/DetialsView/MandateRequest';
import TrainingRequest from '../MyRequest/DetialsView/TrainingRequest';
import Resignation from '../MyRequest/DetialsView/Resignation';
import PurchaseOrderDetail from '../MyRequest/DetialsView/PurchaseOrderDetail';
import FormPaymentOrder from '../MyRequest/DetialsView/FormPaymentOrder';
import FormCertAchievement from '../MyRequest/DetialsView/FormCertAchievement';
import Rhletter from '../MyRequest/DetialsView/Rhletter';
import FormInternalCourses from '../MyRequest/DetialsView/FormInternalCourses';
import CustodyRequest from '../MyRequest/DetialsView/CustodyRequest';
import LeaveRequest from '../MyRequest/DetialsView/LeaveRequest';
import RemoteRequest from '../MyRequest/DetialsView/RemoteRequest';
import CustodyClose from '../MyRequest/DetialsView/CustodyClose';
import NewPurchaseRequest from '../MyRequest/DetialsView/NewPurchaseRequest';
import TechnicalRequest from '../MyRequest/DetialsView/TechnicalRequest';
import PopupActionOrder from '../MyRequest/DetialsView/PopupActionOrder';
import FormPurchaseOrder from '../MyRequest/DetialsView/FormPurchaseOrder';
import FormPurchaseAddBudget from '../MyRequest/DetialsView/FormPurchaseAddBudget';
import TechnicalRequestService from '../MyRequest/DetialsView/TechnicalRequestService';
import TechnicalRequestOld from '../MyRequest/DetialsView/TechnicalRequestOld';
...
const Item = ({ item, menu, menuTab, navigation, scrollToIndex, reload }) => {
  ...
  if (item?.res_model === 'hr.holidays') {
    ...
    viewItem.lineOne = 'خطاب موارد بشرية';
    ...
    viewItem.lineDescription = ' من ' + item.date_from + ' الى ' + item.date_to;
    let type = null;
    if (item.type) {
    ...
    if (menu === 'approval') {
      ...
      viewItem.lineTwo =
        prefixForLineTwo + (item.duration ? item.duration + ' يوم' : null);
    }
    ...
    var val = item.payment_amount
      ? item.payment_amount.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')
      : null;
    viewItem.lineDescription =
      'القيمة: ' + val + ' ' + (item.currency_id ? item.currency_id[1] : null);
    ...
    let name2 =
      ...
      item.employee_id[1]
        ...
        .slice(-2)
        .join(' ');
    if (item.employee_id[1].length >= 30 || item.employee_id[1].length <= 27) {
      name0 = name1;
    } else {
      name0 = name2;
    }
    viewItem.lineTwo =
      menu === 'approval' ? 'الدورة الداخلية ل' + name0 : 'الدورة الداخلية ';
    ...
    viewItem.lineDescription2 = `التصنيف: ${item.category}`;
    ...
    viewItem.lineOne = item.name;
    viewItem.lineThree = Moment(item.custody_order_date).format('DD-MM-YYYY');
    viewItem.lineDescription = item.description
      ? `الوصف: ${item.description}`
      : '';
    viewItem.lineDescription2 = `التصنيف: ${item.type}`;
    viewItem.lineTwo = `${item.id}`;
  }
  //new
  else if (item?.res_model === 'purchase.requisition') {
    ...
    >
      <View
        ...
        style={{
          // flexDirection: 'row',
          height:
            menu !== 'myRequest' && itemClicked != item.id
      ...
      {itemClicked == item.id && (
        ...
        >
          ...
          {item.res_model == 'hr.resignation' && (
            <Resignation item={item} viewType={menu} navigation={navigation} />
          )}
          {item.res_model == 'purchase.add.budget' && (
          ...
          {item.res_model == 'hr.authorization' && (
            <LeaveRequest item={item} viewType={menu} navigation={navigation} />
          )}
          ...
          {item.res_model == 'salary.identification.request' && (
            <Rhletter item={item} viewType={menu} navigation={navigation} />
          )}
          {item.res_model == 'manage.financial.custody' && (
            <CustodyRequest
              item={item}
          ...
          {item.res_model == 'manage.financial.custody.close' && (
            <CustodyClose item={item} viewType={menu} navigation={navigation} />
          )}
          ...
          {itemClicked == item.id && (
            <View
              style={{
                ...
                shadowOpacity: 0.15,
                shadowOffset: { width: 0, height: 0 },
                height: menu !== 'myRequest' ? 70 : 0,
                paddingHorizontal: 22,
                borderBottomRightRadius: 12,
                borderBottomLeftRadius: 12,
                borderColor: '#e8ecee',
                borderWidth: menu !== 'myRequest' ? 1 : 0,
                marginBottom: 1,
              }}
            >
              {menu !== 'myRequest' && (
